...
Deployed at: **https://dp-phased-model.vercel.app**  
...
`window.storage` is **Claude artifact API only** — does not work in deployed Vercel app.  
...
Vercel project: `dp-phased-model`  
Auto-deploys from `main` branch on GitHub (`LIFEAI/dp-phased-model`).  
No build config needed — Vite auto-detected.

To push a change:
1. Edit `src/App.jsx` in `/home/claude/`
2. Push via GitHub API (see prior session pattern) with current SHA
3. Vercel deploys in ~60 seconds
